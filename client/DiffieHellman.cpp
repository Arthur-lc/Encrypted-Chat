...
void DiffieHellman::computeSharedSecret(const BIGNUM* otherPublicKey) {
    ...
    int secretSize = DH_size(dh);
    std::vector<unsigned char> secret(secretSize);
    
    int actualSize = DH_compute_key(secret.data(), otherPublicKey, dh);
    if (actualSize == -1) {
        return;
    }

    // Redimensiona para o tamanho real do segredo
    secret.resize(actualSize);
    
    // Deriva a chave do grupo
    deriveGroupKey(secret);
}

void DiffieHellman::deriveGroupKey(const std::vector<unsigned char>& secretBytes) {
    if (secretBytes.empty()) return;
